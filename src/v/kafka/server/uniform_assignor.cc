...
public:
    ...
    /// Advances to `topic` if present in the tail of the sequence. Returns
    /// where its element is, or `end()`.
    iterator advance_to(model::topic_id topic) {
        while (_next != _end && std::invoke(_proj, *_next) < topic) {
            ++_next;
        }
        if (_next == _end || std::invoke(_proj, *_next) != topic) {
...
private:
    ...
    public:
        ...
        void insert(model::partition_id partition) {
            ...
            _words[word_of(partition)] |= bit_of(partition);
        }
    ...
    std::expected<subscription_index, assignor_error> index_subscriptions() {
        ...
        for (const auto& topic : subscription()) {
            ...
            partition_set unowned;
            for (auto id : std::views::iota(0, *num_partitions)) {
                model::partition_id partition{id};
                if (!claimed.contains(partition)) {
                    unowned.push_back(partition);
                }
            }
            if (!unowned.empty()) {
...
private:
    ...
    std::expected<void, assignor_error> keep_what_is_still_subscribed() {
        ...
        for (auto index : std::views::iota(int32_t{0}, num_members)) {
            ...
            for (const auto& [topic, curr_assigned] :
                 member.current_assignment) {
                if (member_topics.advance_to(topic) == member_topics.end()) {
                    continue;
                }
                auto found = group_topics.advance_to(topic);
                vassert(
                  found != group_topics.end(), "{} is not subscribed", topic);
                if (curr_assigned.empty()) {
                    continue;
                }
                if (
