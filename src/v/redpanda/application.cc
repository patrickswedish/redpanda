...
ss::future<> application::register_cluster_identity_metrics() {
    ...
    // cluster_id is generated by the metrics reporter and propagated through
    // cluster config by the controller leader shortly after bootstrap
    // (available immediately from the local config on restart). Wait for it
    // too, so the metric registers exactly once carrying both identities.
    // Label values are fixed at registration time: a later runtime edit of
    // cluster_id is not reflected until the broker restarts.
    auto cluster_id = config::shard_local_cfg().cluster_id.bind();
    cluster_id.watch([this] { _cluster_identity_cvar.signal(); });
    auto waited = co_await ss::coroutine::as_future(
      _cluster_identity_cvar.wait(
        [&cluster_id] { return cluster_id().has_value(); }));
    if (waited.failed()) {
        // broken at shutdown before cluster_id was initialized
    ...
    _cluster_identity_metrics.add_group(
      ...
      {sm::make_gauge(
         ...
         {sm::label("cluster_uuid")(
          ...
          sm::label("cluster_id")(*cluster_id())})
         .aggregate({sm::shard_label})});
}
